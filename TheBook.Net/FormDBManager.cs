...
namespace TheBook.Net
{
    public partial class FormDBManager : Form
    {
        ...
        public void launchDB()
        {
            ...
            if (journalForm == null)
            {
                // no journal form so create new
                result = createJournalForm();
            }
            else
            {
                if (journalForm.IsDisposed)
                {
                    // form was disposed, so create new form
                    result = createJournalForm();
                }
            }
            if (result)
        ...
        public bool createJournalForm()
        {
            FormSelectDesign obj = new FormSelectDesign();
            if (obj.ShowDialog() != DialogResult.OK) return false;
            if (obj.radioDesign1.Checked)
            {
                journalForm = new DiaryJournal.Net.FormJournalDesign1();
                ((DiaryJournal.Net.FormJournalDesign1)(journalForm)).dbCtx = dbCtx;
                ((DiaryJournal.Net.FormJournalDesign1)(journalForm)).myParentForm = this;
            }
            else
            {
                journalForm = new DiaryJournal.Net.FormJournalDesign2();
                ((DiaryJournal.Net.FormJournalDesign2)(journalForm)).dbCtx = dbCtx;
                ((DiaryJournal.Net.FormJournalDesign2)(journalForm)).myParentForm = this;
            }
            return true;
        }
        public void CloseDB()
