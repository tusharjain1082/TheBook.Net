        ...
        private static void AddImage(XElement xamlParentElement, XElement htmlElement, IDictionary<string, string> inheritedProperties, HtmlToXamlContext context)
        {
            ...
            if (currentProperties.TryGetValue("src", out src) && !string.IsNullOrEmpty(src))
            {
                ...
                else
                {
                    // url
                    /* original
                    Bitmap? bitmap = CoreFramework.DownloadImage(src, "", null);
                    if (bitmap != null)
                    {
                    ...
                    */
                    ...
                    String tmpFileName = CoreFramework.autoCreateInitTmpFile();
                    //Bitmap? bitmap = 
                    var task = CoreFramework.DownloadImageAsync(src, tmpFileName, false,0,0);
                    task.Wait();
                    ...
                    if (bitmap != null)
                    {
                        ...
                        {
                            bitmap.Save(ms, bitmap.RawFormat);
                            //CoreFramework.SaveToPngWpf(bitmap, ms);
