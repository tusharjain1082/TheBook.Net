...
namespace DiaryJournal.Net
{
    public partial class FormTreeDesign2 : Form
    {
        ...
        public bool reloadPath(String path, bool usePath, RegisterItem? item)
        {
            ...
            if (usePath)
            {
                if (!Register.LoadFullPath(ctx, ctx.dbNodeTreeRegistryFile, path, ref registry, true))
                    return false; // critical error abort
            }
            else
            {
                if (!Register.LoadFullPath(ctx, ctx.dbNodeTreeRegistryFile, item, ref registry, true, false, false, false, false))
                    return false; // critical error abort
            }
            ...
            ChildrenRegister.Initialize(ctx, ctx.dbNodeTreeRegistryFile, currentItem.children, currentItem);
            ...
            Register.Lineage(ctx, ctx.dbNodeTreeRegistryFile, currentItem, ref ancestors, true);//, true);
        ...
        public bool reloadChildren(Int64 id, ListView lv)
        {
            ...
            // phase 1 - firstly get the registry node item
            List<RegisterItem>? registry = null;
            if (!Register.LoadFullPath(ctx, ctx.dbNodeTreeRegistryFile, id, ref registry, true))
                return false; // critical error abort
