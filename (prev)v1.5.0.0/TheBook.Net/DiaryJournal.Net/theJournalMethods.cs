    ...
    public static class theJournalMethods
    {
            ...
            Int64 rootParentId)
        {
            ...
            for (int i = 0; i < lineageNames.Count; i++)
            {
                ...
                myNode? node = initializeNodeTJNC(ctx, ref primaryWorkList, parentId, lineageNames[i]);
        ...
        public static myNode? initializeNodeTJNC(OpenFSDBContext? ctx, ref List<myNode> primaryWorkList, Int64 parentId, String title)
        {
            // find if node exists
            ...
            byte[]? xamlbytes = null;
            //node = entryMethods.DBNewNode(ctx, SpecialNodeType.None, NodeType.NonCalendarEntry, DomainType.AnyOrAll,
            //    ref node, true, true, true, DateTime.Now, parentId, true, title, "", xamlbytes, true, false, false);
