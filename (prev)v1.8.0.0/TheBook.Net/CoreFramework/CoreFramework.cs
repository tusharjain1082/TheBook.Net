...
namespace DiaryJournal.Net
{
    public static class CoreFramework
    {
        ...
        public static bool autoCreateInitLocalPaths()
        {
            ...
            myDBPath = Path.Combine(Application.StartupPath, DBDirName);
            DBDirConfig = Directory.CreateDirectory(myDBPath);
            if (DBDirConfig == null) return false;

            // done
            return true;
        }

        public static Stream? createTempWorkFile(out String fileOut)
        {
            String path = Path.Combine(myTempPath, Guid.NewGuid().ToString());
            fileOut = path;
            try
            {
        ...
        public static bool removeTempWorkFile(String file)
        {
            try
            {
                File.Delete(file);
        ...
        // format file name with this moment's date and time
