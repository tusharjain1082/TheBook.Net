...
namespace DiaryJournal.Net
{
    ...
    public BitmapImage LoadImageWithCorrectDPI(string imagePath, double dpiX, double dpiY)
    {
        BitmapImage bitmap = new BitmapImage();
        bitmap.BeginInit();
        bitmap.UriSource = new Uri(imagePath);
        bitmap.CacheOption = BitmapCacheOption.OnLoad;
        bitmap.DecodePixelWidth = (int)(bitmap.PixelWidth * (96.0 / dpiX)); // Adjust based on original DPI
        bitmap.DecodePixelHeight = (int)(bitmap.PixelHeight * (96.0 / dpiY)); // Adjust based on original DPI
        bitmap.EndInit();
        return bitmap;
    }

    public void formatInsertImageWpf(WpfRichTextBoxEx rtb, System.Windows.Media.Imaging.BitmapImage bmp)
        {
            ...
            image.Source = bmp;
            image.Stretch = Stretch.Uniform;// Stretch.Uniform; // or other Stretch options
            if (bmp.Width > 600)
            {
                image.Width = 600; // Example fixed width
                image.Height = double.NaN; // Example fixed height
            }
            else
            {
                image.Width = bmp.Width; // Example fixed width
                image.Height = bmp.Height; // Example fixed height
            }
            ...
            {
                rtb.CaretPosition.Paragraph.Inlines.Add(uiContainer);
            }
            else // If no paragraph exists, create one
            {
                Paragraph newParagraph = new Paragraph();
                newParagraph.Inlines.Add(uiContainer);
                rtb.Document.Blocks.Add(newParagraph);
            }
        }
