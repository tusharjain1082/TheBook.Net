...
namespace DiaryJournal.Net
{
    ...
    public partial class FrmJournal : Form
    {
        ...
        private void FrmJournal_Load(object sender, EventArgs e)
        {
            ...
            //bool result = await myCommonMethods1.SendPing("meter.net");
            //if (!result)
            ...
            this.Text = $"Tushar Jain's XTS-Aes encryption secured TheBook.Net-Version{Application.ProductVersion}::DiaryJournal.Net-Version{version}, uses .Net 8.0, Compiled/Built on: " + strBuildDateTime;
        ...
        private void TvEntries_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            myNode? targetNode = entryMethods.FindNodeInList(allNodes, targetNodeID);
            ...
            myNode? draggedNode = entryMethods.FindNodeInList(allNodes, draggedNodeID);
            ...
            if (!draggedTreeNode.Equals(targetTreeNode) && !ContainsNode(draggedTreeNode, targetTreeNode))
            {
                ...
                {
                    if (!entryMethods.DBSetNodeParent(dbCtx, ref draggedNode, targetNode.chapter.Id))
                        return;
                    ...
                    // rebuild lineage
                    draggedNode.lineage = entryMethods.findBottomToRootNodesRecursive(allNodes, ref draggedNode, false, true, true);
                }
        ...
        public void applyConfig()
        {
            ...
            if (dbCtx.config.tvEntriesItemHeight <= 0) cfg.tvEntriesItemHeight = myConfig.default_tvEntriesItemHeight;
            if (dbCtx.config.tvEntriesIndent <= 0) cfg.tvEntriesIndent = myConfig.default_tvEntriesIndent;
            ...
            if (dbCtx.config.tvEntriesFont == null) cfg.tvEntriesFont = myConfig.default_tvEntriesFont;
            tvEntries.Font = cfg.tvEntriesFont;
            linkCfgTVEntriesFont.Text = myCommonMethods1.FontToString(dbCtx.config.tvEntriesFont);

            if (dbCtx.config.tvEntriesBackColor == Color.Empty) cfg.tvEntriesBackColor = myConfig.default_tvEntriesBackColor;
            if (dbCtx.config.tvEntriesForeColor == Color.Empty) cfg.tvEntriesForeColor = myConfig.default_tvEntriesForeColor;
        ...
        public void viewEntry(myNode node)
        {
            String rtf = (String?)entryMethods.DBLoadNodeData(dbCtx, node.chapter.Id, node.DirectorySectionID);
        ...
        public void OpenSearchedEntry(Int64 id, bool openParentNode = false)
        {
            ...
            if (openParentNode)
            {
                node = entryMethods.FindNodeInList(allNodes, node.chapter.parentId);
                if (node == null)
                    return;
            }
            ...
            String title = "")
        {
            ...
            entryMethodsNewDesign.DBCreateLoadYearSystemNode(dbCtx, RootSystemNodesRegistry, ref emptySlotsItem, dateTime, ref year);
            // create-load month node
            entryMethodsNewDesign.DBCreateLoadMonthSystemNode(dbCtx, ref emptySlotsItem, year, dateTime, ref month);

            /* todo tushar: 02 January 2025
            myNode? parentSystemNode = systemNodes.getSystemNode(parentSystemNodeType);
            myNode? parentNode = null;
            if (parentId > 0)
            {
                parentNode = entryMethods.FindNodeInList(allNodes, parentId);
                if (parentNode == null)
                    return;
            }
            else if (parentId == 0)
            ...
            List<myNode> lineageChain = entryMethods.findBottomToRootNodesRecursive(allNodes, ref node, false, true, true);
            ...
            String title = "")
        {
            ...
            byte[]? xamlbytes = null;
            RegisterItem? item = Register.Insert(dbCtx, dbCtx.dbNodeTreeRegistryFile, parent.Id, ref emptySlotsItem, ref node, "", xamlbytes);
            if (item == null)
            {
            ...
            // add the newly created node in the context session work list
            if (allNodes.Find(x => x.chapter.Id == year.Id) == null) allNodes.Add(year.node);
            if (allNodes.Find(x => x.chapter.Id == month.Id) == null) allNodes.Add(month.node);
            allNodes.Add(node);
            ...
            // update ui
            __setCalendarHighlightEntry(node.chapter.chapterDateTime);
            tsslblTotalEntries.Text = allNodes.LongCount().ToString();
            tvEntries.SelectedNode = newTreeNode;
            //lineageChain.Reverse();
            //node.lineage = lineageChain;
        }
        ...
        public bool reloadAll(bool autoAllNodes, bool autoSystemNodes, bool autoTreeView, bool autoLineage)
        {
            // todo everything
            ...
            if (!entryMethodsNewDesign.DBCreateRootSystemNodes(dbCtx))
            {
                this.Enabled = true;
                formOperation.close();
            ...
            // reload all nodes and lineages afresh
            if (autoAllNodes) allNodes = entryMethods.DBFindAllNodes(dbCtx, false, false);
            if (autoLineage) entryMethods.DBReloadLineages(allNodes, true, true, true);
        ...
        public void __treeViewSort(TreeView tv)
        {
            ...
            if (dbCtx.radCfgUseOpenFileSystemDB)
                nodes = OpenFileSystemDB.findAllNodes(dbCtx, true, false);
            else
                nodes = SingleFileDB.findAllNodes(dbCtx.ctx0, true, false);
        ...
        */
        ...
        public TreeNode? __initTreeViewEntry(ref myNode nodeEntry)
        {
            ...
            if (nodeEntry.chapter.parentId == 0)
            {
                ...
                if (matchingNodes.Length == 0) // node does not exists, so create it
                {
                    ...
                    newNode.Name = path;
                    loadNodeHighlight(dbCtx, newNode, ref nodeEntry);
        ...
        public List<myNode> __loadTree(ref List<myNode> srcNodes)
        {
            ...
            this.Invoke(updateTotalEntriesStatus, srcNodes.LongCount());
            return outputTree;
        }
        ...
        public void __saveEntry()
        {
            ...
            entryMethods.DBUpdateCaretConfig(dbCtx, ref node, rtbEntry.SelectionStartOffset, rtbEntry.SelectionLength);
            ...
            // save the entry
            entryMethods.DBUpdateNodeOFSDB(dbCtx, ref node, rtbEntry.Rtf, rtbEntry.XamlBytes, true, true, true);
        ...
        public void setupEntryTitle(String title)
        {
            ...
            if (!entryMethods.DBUpdateNodeTitle(dbCtx, node, title))
                return;
            ...
            if (entryMethods.GenerateLineagePath(allNodes, ref node, out entryPath, out lineage))
        ...
        public void __gotoEntry(DateTime dateTime)
        {
            ...
            List<myNode> list = entryMethods.findNodesByNodeTypeDate(allNodes,
        ...
        public void __gotoEntryByAttribute(bool lm, bool lc, bool tc, bool byID, Int64 id)
        {
            ...
            {
                if (id <= 0) return;
                node = entryMethods.FindNodeInList(allNodes, id);
                if (node == null)
                    return;
            }
        ...
        public void insertImage()
        {
            ...
            Uri uri = new Uri(ofdFile.FileName, UriKind.RelativeOrAbsolute);
        ...
        public void findTotalEntriesInDB()
        {
            ...
            MessageBox.Show("total number of " + allNodes.LongCount() + " validated/valid entries actually exist in db.", "status", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        public void deleteEntries()
        {
            ...
            entryMethods.DBDeleteOrPurgeListRecursive(dbCtx, ref worklist, ref nodes, true, false, false);
        ...
        public void refreshTrashcan()
        {
            ...
            List<myNode> nodes = entryMethods.DBFindDeletedNodes(allNodes);
            foreach (myNode node in nodes)
        ...
        public void purgeTrashCan()
        {
            ...
            foreach (System.Windows.Forms.ListViewItem listViewItem in lvTrashCan.CheckedItems)
            {
                ...
                entryMethods.DBDeleteOrPurgeNodeRecursive(dbCtx, ref allNodes, ref node, false, true, false);
            }
        ...
        public void restoreTrashCan()
        {
            ...
            foreach (System.Windows.Forms.ListViewItem listViewItem in lvTrashCan.CheckedItems)
            {
                ...
                entryMethods.DBDeleteOrPurgeNodeRecursive(dbCtx, ref allNodes, ref node, false, false, false);
            }
        ...
        public void highlightCheckedEntries(bool reset)
        {
            ...
            //if (dbCtx.ctx0.isDBOpen())
            //    SingleFileDB.Checkpoint(dbCtx.ctx0);
        ...
        public void highlightTreeViewNode(TreeNode treeNode, System.Drawing.Font font, System.Drawing.Color color, System.Drawing.Color backColor, bool reset)
        {
            ...
            if (reset)
                entryMethods.setEntryClearHighlight(dbCtx, ref node);
            else
                entryMethods.setEntryHighlightFontComplete(dbCtx, ref node, color, backColor, font);
        ...
        public void highlightEntryBackColor(TreeNode treeNode)
        {
            ...
            entryMethods.setEntryHighlightBackColor(dbCtx, ref node, colorDialog.Color);
            loadNodeHighlight(treeNode);
        }
        ...
        public bool __processSearch()
        {
            ...
            /*
            return journalSearchFramework.searchEntries(dbCtx, ref allNodes, this, txtSearchProgressFullPath, lvSearch, tsSearchProgressBar,
        ...
        public void restoreSearchedList()
        {
            ...
            foreach (System.Windows.Forms.ListViewItem listViewItem in lvSearch.CheckedItems)
            {
                ...
                entryMethods.DBDeleteOrPurgeNodeRecursive(dbCtx, ref worklist, ref node, false, false, false);
        ...
        public void deleteSearchedList()
        {
            ...
            foreach (System.Windows.Forms.ListViewItem listViewItem in lvSearch.CheckedItems)
            {
                ...
                entryMethods.DBDeleteOrPurgeNodeRecursive(dbCtx, ref worklist, ref node, true, false, false);
        ...
        public void gotoSystemNode(NodeType nodeType)
        {
            // auto create/load system nodes
            mySystemNodes? systemNodes = null;
            //List<myNode> allNodes = entryMethods.DBFindAllNodes(dbCtx, false, false);
            if (!entryMethods.autoCreateLoadSystemNodes(dbCtx, ref allNodes, out systemNodes))
                return;
        ...
        public void __exportEntries(EntryType entryType, bool loadOperationForm)
        {
            ...
            treeDom.buildCustomTree(allNodes, ref checkedNodes, true, false);
        ...
        public void promoteNode()
        {
            ...
            if (!entryMethods.DBPromoteNodeOFSDB(dbCtx, node))
                return;

            // rebuild lineage
            node.lineage = entryMethods.findBottomToRootNodesRecursive(allNodes, ref node, false, true, true);
        ...
        public void promoteNodeToRoot()
        {
            ...
            // todo tushar
            //if (!entryMethods.DBPromoteNodeToRoot(dbCtx, ref node))
            //    return;
        ...
        public void __moveNodeTo(TreeNode? subjectTreeNode)
        {
            ...
            myNode? subjectNode = entryMethods.FindNodeInList(allNodes, subjectNodeId);
            if (subjectNode == null)
                return;
            ...
            // rebuild lineage
            subjectNode.lineage = entryMethods.findBottomToRootNodesRecursive(allNodes, ref subjectNode, false, true, true);
        ...
        public void doExportCustomEntry(Int64 id)
        {
            ...
            sfdFile.FileName = entryMethods.getEntryLabel(node, true, true);
            ...
            // straightaway export the entry
            if (entryMethods.exportEntry(dbCtx, ref node, sfdFile.FileName, true, 0, entryType))
        ...
        private void create30000TestNodesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (index = 0; index < total; index++)
            {
                ...
                byte[]? xamlbytes = null;
                entryMethods.DBCreateNode(dbCtx, ref node, "", xamlbytes, true, true, true, true, false, false);
        ...
        public void __importTheJournalNonCalendarRtfEntriesNew(String path, String importSetName, bool loadOperationForm)
        {
            ...
            foreach (String file in files)
            {
                ...
                List<myNode> nodesLine = theJournalMethods.initNodesLineTJNC(dbCtx, ref setList, ref nodeNames, setNode.chapter.Id);
                ...
                entryMethods.DBUpdateNodeOFSDB(dbCtx, ref targetNode, rtf, xamlEntry.dummy.XamlBytes, true, true);
        ...
        private void timerSetRtbEntry_Tick(object sender, EventArgs e)
        {
            ...
            rtf = (String?)entryMethods.DBLoadNodeData(dbCtx, id, node.DirectorySectionID);
            ...
            String entryPath = "";
            List<myNode>? lineage = null;
            if (entryMethods.GenerateLineagePath(node.lineage, ref node, out entryPath, out lineage))
                txtBoxEntryPath.Text = entryPath;
        ...
        public void configureEntryWidth(System.Windows.Controls.WpfRichTextBoxEx rtb, Int64 id)
        {
            ...
            // todo tushar
            //if (!entryMethods.DBSetNodeDocumentWidth(dbCtx, ref node, value))
            //    return;
        ...
        public void __selectSearchNode()
        {
            ...
            foreach (myNode node in selNodes)
            {
                ...
                foreach (ListViewItem listedItem in lvSearchWhere.Items)
                {
                    ...
                    if (entryMethods.IsAncestorNode(allNodes, listedItemId, node, false) == 1)
            ...
            __selectSearchNodeDo:
                ...
                String entryPath = "";
                List<myNode>? lineage = null;
                if (entryMethods.GenerateLineagePath(listedNode.lineage, ref listedNode, out entryPath, out lineage))
                    item.Text = entryPath;
        ...
        private void aboutToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            String text = $"Tushar Jain's TheBook.Net-Version{Application.ProductVersion}::DiaryJournal.Net-Version{version}" +
                "\nan XTS-Aes encryption secured open source free writing sotware a must for books and stories writers, notes keeping, books texts and stories collection and scribing, diary and journal software, uses x64 platform, .Net 8.0 and Visual Studio 2022.\n" +
                "Copyright © 2022 - 2026, Tushar Jain\nsole developer: Tushar Jain";
        ...
        public bool cloneEntry(TreeNode? treeNode, Int64 locationId)
        {
            ...
            if (!dbCtx.isDBOpen()) return false;
        ...
        private void resetCheckedNodesRecursiveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            entryMethods.DBCustomizeTreeNodesRecursive(dbCtx, ref checkedNodes, false,
        ...
        private void resetAllDbNodesRecursiveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            allNodes = entryMethods.DBFindAllNodes(dbCtx, false, false);

            // customize commit nodes
            entryMethods.DBCustomizeTreeNodesRecursive(dbCtx, ref allNodes, false,
                form.resetFont, form.resetFontSize, form.resetItalics, form.resetBold, form.resetStrikeout, form.resetUnderline,
                form.resetBackColor, form.resetForeColor, Color.Empty, Color.Empty, "", -1);
        ...
        private void nullifyemptyAllFontAndColorEntireDbToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // process
            long processed = 0;
            entryMethods.DBNullOrEmptyNodes(dbCtx, ref allNodes, out processed, formOperation);
        ...
        private void toolStripMenuItem82_Click(object sender, EventArgs e)
        {
            ...
            String rtf = entryMethods.DBLoadNodeData(dbCtx, node.chapter.Id, node.DirectorySectionID);
        ...
        private void setEntryMemoryToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!dbCtx.isDBOpen())
                return;
            ...
            VirtualDiskFramework.EntrySharedMemoryFile obj = new VirtualDiskFramework.EntrySharedMemoryFile();
            ...
            String filename = entryMethods.getEntryLabel(node, true, true) + ".rtf";
            // load chapter's data blob
            String rtf = (String?)entryMethods.DBLoadNodeData(dbCtx, node.chapter.Id, node.DirectorySectionID);

            // serialize and set to memory mapped file
            obj.label = filename;
            obj.rtf = rtf;
            String xmlData = obj.Serialize();
            ...
            try
            {
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    String name = $"TheJournal.Net-SharedEntryFile";
                    if (sharedEntry != null) sharedEntry.Dispose();
                    sharedEntry = MemoryMappedFile.CreateNew(name, bytes.Length);
                    using (MemoryMappedViewStream mmfvstream = sharedEntry.CreateViewStream())
                    {
                        mmfvstream.Write(bytes);
                        mmfvstream.Flush();
                    }
                }
            }
            ...
            MessageBox.Show("successfully set shared entry. you can import it as file in file manager.", "done", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
