...
namespace DiaryJournal.Net
{
    ...
    public static class myCommonMethods1
    {
        ...
        public static BitmapImage? ToBitmapImage(BitmapSource? bitmapSource)
        {
            ...
            {
                // 1. Encode the BitmapSource to a stream
                BitmapEncoder encoder = new PngBitmapEncoder();
                encoder.Frames.Add(BitmapFrame.Create(bitmapSource));
                encoder.Save(memoryStream);

                // 2. Create the BitmapImage and load it from the stream
                BitmapImage bitmapImage = new BitmapImage();
                bitmapImage.BeginInit();
                bitmapImage.CacheOption = BitmapCacheOption.OnLoad; // Ensures the stream can be closed
                bitmapImage.StreamSource = new MemoryStream(memoryStream.ToArray());
                bitmapImage.EndInit();

                // Ensure the image is ready for use (especially in multi-threaded scenarios)
                bitmapImage.Freeze();
        ...
        public static BitmapSource? GetWPFImageFromClipboard()
        {
            if (System.Windows.Clipboard.ContainsImage())
            {
                // Retrieve the image as a BitmapSource
                BitmapSource? imageSource = System.Windows.Clipboard.GetImage();
                return imageSource;

                // Example: Assign to an Image control
        ...
        public static Image? GetImageFromClipboard()
        {
            ...
            {
                // Retrieve the image
                Image? clipboardImage = System.Windows.Forms.Clipboard.GetImage();
                return clipboardImage;

                // Example: Display in a PictureBox or save to file
                // pictureBox1.Image = clipboardImage;
                // clipboardImage.Save("clipboard_image.png", System.Drawing.Imaging.ImageFormat.Png);
            }
            else
            {
                return null;
