...
namespace DiaryJournal.Net
{
    public static class entryMethods
    {
        ...
        public static String getFormattedPathFileName(String path, UInt32 id, UInt32 parentId, String title,
            DateTime dateTime, UInt32 exportIndex, EntryType entryType, out String entryNameOut)
        {
        ...
        public static String getEntryLabel(myNode node, bool removeInvalidPathChars, bool insertRevDateTime = false)
        {
            ...
            switch (node.chapter.nodeType)
            {
                ...
                */
                ...
                case NodeType.Entry:
                    // new change on 14 Feb 2026
                    entryName = String.Format(@"({0}):({1}:{2}:{3}:{4}):({5}):(ID {6})", node.chapter.chapterDateTime.ToString("dddd-dd-MMMM-yyyy"),
                        node.chapter.chapterDateTime.Hour, node.chapter.chapterDateTime.Minute,
                        node.chapter.chapterDateTime.Second, node.chapter.chapterDateTime.Millisecond, node.chapter.Title, node.chapter.Id.ToString());
                    break;
        ...
        public static bool DBPromoteNodeOFSDB(OpenFSDBContext ctx, myNode? node)
        {
            ...
            RegisterItem? currentItem = Register.LoadSetupRegisterItem(ctx, node.chapter.Id, true, false, false, false, false, false);
            if (currentItem == null) return false;
            RegisterItem? parent = Register.LoadSetupRegisterItem(ctx, currentItem.parentId, true, false, false, false, true, true);
            if (parent == null) return false;
            RegisterItem? dst = Register.LoadSetupRegisterItem(ctx, parent.parentId, true, false, false, false, true, true);
        ...
        public static void DBExploreEntryFileOFSDB(OpenFSDBContext? ctx, UInt32 sectionId, UInt32 id)
        {
            OpenFileSystemDB.exploreNodeEntryFile(ctx, sectionId, id);
        }

        // this promotes the node to one level up in tree structure.
        public static bool DBChangeNodeParentOFSDB(OpenFSDBContext ctx, UInt32 sectionId, UInt32 id, UInt32 newParentId)
        {
        ...
        public static bool DBSetNodeParent(OpenFSDBContext? ctx, myNode? node, UInt32 parentId)
        {
            if (node == null)
                return false; // error node not found
        ...
        public static Object? DBLoadNodeData(OpenFSDBContext? ctx, UInt32 id, UInt32 sectionId = 0)
        {
        ...
        public static myNode? DBFindLoadNode(OpenFSDBContext? ctx, UInt32 id, ref String? rtf, ref byte[] xaml, bool loadData = false, UInt32 sectionId = 0)
        {
            return OpenFileSystemDB.findLoadNode(ctx, sectionId, id, ref rtf, ref xaml, loadData);
        }
        // find a node by id
        public static myNode? DBFindLoadNodeOFSDB(OpenFSDBContext ctx, UInt32 id, ref String rtf, bool loadData = false, UInt32 sectionId = 0)
        {
        ...
        // search a node by id
        public static myNode? DBSearchNodeOFSDB(OpenFSDBContext ctx, UInt32 id, ref String? rtf, ref byte[]? xamlbytesOut, bool loadData = false)
        {
        ...
        // auto select db and find a node by id
        public static myNode? FindNodeInList(List<myNode> allNodes, UInt32 id)
        {
            foreach (myNode node in allNodes)
            {
        ...
        // auto select db and find a node by title
        public static myNode? BuildPathNodes(ref UInt32 startingNodeIndex, String path, String delimiter, ref List<myNode> listOut)
        {
        ...
        public static int IsAncestorNode(List<myNode> allNodes, UInt32 nodeToCheckId, myNode srcNode, bool deleted = false)
        {
        ...
        public static bool DBPurgeNodeOFSDB(OpenFSDBContext ctx, UInt32 id, UInt32 sectionId, bool purgeConfig = true, bool purgeData = true)
        {
        ...
        public static void setNodeID(ref List<myNode> allNodes, myNode node, UInt32 newID, bool setChildrenParentID = true)
        {
        ...
        public static RegisterItem? DBSetNodeCommonDateTimeOFSDB(OpenFSDBContext ctx, UInt32 id, DateTime newDateTime)
        {
            RegisterItem? item = Register.LoadSetupRegisterItem(ctx, id, true, false, false, false, false, false);
        ...
        public static bool DBExportNodeTree(OpenFSDBContext? ctx, UInt32 id, String path, EntryType OutputEntryType, FormOperation? formop = null)
        {
            UInt32 i = 0;

            // first export ancestor
            RegisterItem? ancestor = Register.LoadSetupRegisterItem(ctx, id, true, false, false, false, true, true);
            if (ancestor == null) return false;
            entryMethods.exportEntry(ctx, ref ancestor.node, path, false, i++, OutputEntryType);
            ...
            while (true)
            {
                ...
                nextDescendant = Register.LoadSetupRegisterItem(ctx, nextDescendant.Id, true, false, false, false, true, true);
                if (nextDescendant == null) return false;

                // export this descendant node as document
                entryMethods.exportEntry(ctx, ref nextDescendant.node, path, false, i++, OutputEntryType);
            }
            return true;
        }
        ...
        public static bool exportEntry(OpenFSDBContext? ctx, ref myNode? node, String path, bool useCustomPathFileName,
            UInt32 exportIndex, EntryType OutputEntryType)
        {
            if (node == null)
                return false;
        ...
        public static bool validateExtractEntryFile(String file, ref UInt32 index, ref DateTime chapterDate,
            ref String title, ref UInt32 id, ref UInt32 parentId)
        {
            ...
            if (!UInt32.TryParse(matches2[0].Groups[1].Value, out index))
                return false;
        ...
        public static String findEntryFileByExportIndex(IEnumerable<String> files, UInt32 index)
        {
            DateTime chapterDate = DateTime.Now;
            String title = "";
            UInt32 foundIndex = 0;
        ...
        public static RegisterItem? DBCloneNodeOFSDB(OpenFSDBContext ctx, UInt32 id, UInt32 locationId, ref RegisterItem? emptySlots)
        {
            if (id == 0) return null; // error root cannot be cloned

            // get latest state register item
            RegisterItem? item = Register.LoadSetupRegisterItem(ctx, id, true, true, false, false, false, false);
            ...
            // configure
            UInt32 parentId = item.node.chapter.parentId;
            if (locationId != -1) parentId = locationId;

            // reconfigure node and insert to create a clone
            item.node.chapter.Id = 0;
            item.node.chapter.parentId = parentId;
			RegisterItem? cloneItem = Register.Insert(ctx, parentId, emptySlots, item.node, item.rtf, item.xamlbytes);
            return cloneItem;
        }
        ...
        public static bool DBNodesConvertToRawTextOFSDB(OpenFSDBContext? ctx, out UInt32 processed, FormOperation? formop = null)
        {
            processed = 0;

            // first get the total number of chapters which exist in db
            UInt32 total = Register.Total(ctx);
            UInt32 index = 0;
        ...
        public static bool DBConvertOFSDB(OpenFSDBContext? ctx, 
            bool fromRtfToXaml, out UInt32 processed, FormOperation? formop = null)
        {
            processed = 0;
        ...
        // this method imports documents
        public static bool DBImportDocumentsOFSDB(OpenFSDBContext? ctx, UInt32 parentId, RegisterItem emptySlotsItem, String path, FormOperation? formop = null)
        {

            // first get the latest state parent
            RegisterItem? parent = Register.LoadSetupRegisterItem(ctx, parentId, false, false, false, false, true, true);
            if (parent == null) return false;

            Int64 total = Directory.EnumerateFiles(path, "*.*").LongCount();
            Int64 index = 0; 
            ...
            foreach (String file in Directory.EnumerateFiles(path, "*.*"))
            {
                ...
                xamlEntry.dummy.Document.Blocks.Clear();
                ...
                try
                {
                    ...
                    {
                        // valid document
                        xamlEntry.dummy.Rtf = File.ReadAllText(file);
                    }
                    else if (fileInfo.Extension == ".txt")
                    {
                        // valid document
                        xamlEntry.dummy.Text = File.ReadAllText(file);
                    }
                    else if (fileInfo.Extension == ".xaml")
                    {
                        // valid document
                        xamlEntry.dummy.XamlBytes = File.ReadAllBytes(file);
                    }
                    else
                    {
                        // some other file skip
                        continue;
                    }
                }
                catch (Exception)
                ...
                node.chapter.Title = fileInfo.Name;

                // now create registry and node
                RegisterItem? item = Register.Insert(ctx, parent.Id, emptySlotsItem, node, xamlEntry.dummy.Rtf, xamlEntry.dummy.XamlBytes);
            }
            return true;
        }
